            ...
            [
                {<<"Examples from challenge definition">>,
                    fun() ->
                        ...
                        {ok, {{"HTTP/1.1",200,"OK"}, _HeadersSorted, ResponseForSorted}} = httpc:request(post, {?SORTED_URL, Headers, "application/json", InputJson}, [], []),
                        ?assertEqual(ExpectedJsonReply, ResponseForSorted),

                        {ok, {{"HTTP/1.1",200,"OK"}, _HeadersBash, ResponseForBash}} = httpc:request(post, {?BASH_SCRIPT_URL, Headers, "application/json", InputJson}, [], []),
