...
produce_output(sorted, SortedTasks) ->
    {ok, thoas:encode(#{tasks => SortedTasks})};

produce_output(bash_script, SortedTasks) ->
...
-spec sort(Task, KeyedMap, Acc, Seen) -> Result when
    Task        :: task(),
...
sort(#{name := Name} = Task, KeyedMap, Acc, Seen) ->
            ...
            {NewAcc, NewSeen} = lists:foldl(fun(Dep, {DAcc, DSeen}) ->
                case maps:get(Dep, KeyedMap, undefined) of
                    undefined ->
                        {DAcc, DSeen};
                    DepTask ->
                        sort(DepTask, KeyedMap, DAcc, DSeen)
                end
